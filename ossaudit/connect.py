"""Native cognis-connect emit for ossaudit — forward findings to any platform.

Maps ossaudit's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    ossaudit ... --format json | ossaudit-emit --to stix
    ossaudit-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = rec.get('name', '') + ' ' + rec.get('spdx_id', '')
        out['severity'] = rec.get('severity', 'info')
        out['type'] = rec.get('type', 'license')
